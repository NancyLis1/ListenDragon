...
上传成功后任务依次经历 `QUEUED -> EXTRACTING -> TRANSCRIBING -> CHUNKING -> INDEXING -> READY/FAILED`。上传路由仅用 FFprobe 读取媒体元数据；Worker 使用 FFmpeg 提取 16 kHz 单声道 WAV、faster-whisper 生成带时间戳转写，随后切分文本并原子发布 FAISS/BM25 索引。路由不得直接执行转写或生成模型。
